...
public class NetControl extends AbstractVerticle {
    ...
    public void start() throws Exception {
        ...
        vertx.createNetServer() //creates a simple TCP (4th LAYER - OSI)
    ...
    private void handleClient(NetSocket socket) {
        ...
        RecordParser.newDelimited("\n", socket) //parse by looking for new lines
                .handler(buffer -> handleBuffer(socket, buffer)) //each line is a buffer because of above logic
                .endHandler(v -> logger.info("Connection ended"));
    }

    private void handleBuffer(NetSocket socket, Buffer buffer) {
        String command = buffer.toString(); //buffer to string => decodes to the default charset
        switch (command) {
            case "/list":
                listCommand(socket);
                break;
