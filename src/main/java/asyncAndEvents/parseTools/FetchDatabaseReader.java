...
public class FetchDatabaseReader {
  ...
  public static void main(String[] args) {
    ...
    parser.pause();//pausing the push mode
    parser.fetch(1);//FETCH the FIRST preconfigured "4" bytes corresponding to the "magic number" defined in the DB write example.
    parser.handler(header -> readMagicNumber(header, parser));//define the version
  ...
  private static void readMagicNumber(Buffer header, RecordParser parser) {
    ...
    parser.handler(version -> readVersion(version, parser));//defines the handler
    parser.fetch(1);//then fetch which is handled previously ...
  }
